...
if getgenv().RealityHubExecuted then return end
getgenv().RealityHubExecuted = true
...
--------------- Files
makefolder("Reality Hub")
makefolder('Reality Hub\\Anime Adventures')


local DefaultFiles = {

	['Reality Hub\\Settings_' .. player.Name] = {
...
local Settings = HttpService:JSONDecode(readfile('Reality Hub\\Settings_' .. player.Name)) 

local function Save (valueName, newValue)
	Settings[valueName] = newValue
	writefile('Reality Hub\\Settings_' .. player.Name, HttpService:JSONEncode(Settings))
end

local function GetSave (valueName)
	local value = Settings[valueName]
	if value == nil then
		if DefaultFiles['Reality Hub\\Settings_' .. player.Name][valueName] ~= nil then
			Save(valueName, DefaultFiles['Reality Hub\\Settings_' .. player.Name][valueName])
		else
...
HubTitle.Text = 'Reality HUB - BETA'
...
function getMacroUnits(macroName, textBlocks)
	...
	local macroExist = isfile('Reality Hub\\Anime Adventures\\' .. nameMacro)
	local macroExistjson = isfile('Reality Hub\\Anime Adventures\\' .. nameMacro .. '.json')
	...
	if macroExist or macroExistjson then
		...
		local sucess, response = pcall(function()
			local MacroAbout = HttpService:JSONDecode( readfile( 'Reality Hub\\Anime Adventures\\' .. nameMacro) )
...
HideUHLabel.Text = 'Hide Reality Hub'
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['color'] = 11513855,
				['footer'] = {
					['text'] = string.format("// Made by Reality Hub (%s)", Time), 
				},
				['fields'] = {
					{
						['name'] = 'Test Webhook',
...
for tabName, mapsList in pairs(macroMapList) do
	...
	for _, mapName in ipairs(mapsList) do
		...
		newDDLMacroMap.MouseButton1Click:Connect(function()
			...
			local MacrosList = listfiles('Reality Hub\\Anime Adventures')
...
local function RecordMacroFunc ()
	...
	local macroExist = isfile('Reality Hub\\Anime Adventures\\' .. MacroFile )
	local macroExistJson = isfile('Reality Hub\\Anime Adventures\\' .. MacroFile .. '.json')

	if macroExistJson then MacroFile = 'Reality Hub\\Anime Adventures\\' .. MacroFile .. '.json'
	elseif macroExist then MacroFile = 'Reality Hub\\Anime Adventures\\' .. MacroFile
...
createMacroBox.FocusLost:Connect(function(enterPressed)
	...
	writefile('Reality Hub\\Anime Adventures\\' .. createMacroBox.Text .. '.json', HttpService:JSONEncode({}))
end)
...
deleteMacroButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Reality Hub\\Anime Adventures\\' .. item)
	local macroExistjson = isfile('Reality Hub\\Anime Adventures\\' .. item .. '.json')
	if macroExistjson then item = item .. '.json' end

	if not macroExist and not macroExistjson then return end

	delfile('Reality Hub\\Anime Adventures\\' .. item)
...
equipMacroUnitsButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('Reality Hub\\Anime Adventures\\' .. macroSelected)
	local macroExistjson = isfile('Reality Hub\\Anime Adventures\\' .. macroSelected .. '.json')
	...
	local MacroAbout = HttpService:JSONDecode( readfile( 'Reality Hub\\Anime Adventures\\' .. macroSelected) )
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['footer'] = {
					['text'] = string.format("// Made by Reality Hub (%s)", os.date("%X")), 
				},
				['fields'] = {
					{
						['name'] = "Player Stats",
...
task.spawn(function() 
	pcall(function()
		...
		if queue_on_teleport then
			local RealityHubSCRIPT = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/Realityhub7/Reality-Hub/main/Main.lua"))()'
			queue_on_teleport(RealityHubSCRIPT)
		end
